...
| **npm** | Drop `.tgz` files older than N days under `${MIRRORET_BASE_DIR}/npm/approved` (Verdaccio's `storage:` path) and `approved/npm`. When the directory is live Verdaccio storage, files are only deleted with `MIRRORET_NPM_PRUNE_STORAGE=1`; otherwise retention warns and tells you to `npm unpublish` instead, because raw deletion leaves Verdaccio metadata advertising versions that then 404. |
...
`MIRRORET_CLEANUP_HOUR` and `MIRRORET_CLEANUP_DOW`. Then
`sudo ./install.sh --upgrade` to refresh the crontab.

`cleanup-all.sh` also deletes sync logs in `/srv/mirroret/logs/` older than
`MIRRORET_LOG_KEEP_DAYS` (default 30; `0` disables), independent of
`MIRRORET_RETENTION_ENABLE`.

### Watching disk

```bash
sudo du -sh /srv/mirroret/*
mirroretctl clean report          # dry run
sudo mirroretctl clean prune      # delete (needs RETENTION_ENABLE=1)
sudo /srv/mirroret/scripts/cleanup-all.sh   # what cron runs
```
...
- It doesn't rotate logs. Rotation is handled by the managed
  `/etc/logrotate.d/mirroret` that `install.sh` writes (weekly, keep 8, for
  `/srv/mirroret/logs/*.log`; monthly, keep 6, for the install/uninstall
  logs); age-out by `MIRRORET_LOG_KEEP_DAYS` is done by `cleanup-all.sh`.
- It doesn't garbage-collect Verdaccio's `.cache/` under its storage path. That's Verdaccio-internal state.
- It never touches the APT tree (see the top of this document).
...
- nginx vhost config
- systemd unit files (`pypiserver`, `verdaccio`, `mirroret-cache`)
- verdaccio config
- target specs under `/etc/mirroret/targets/` and the cache route table
- `sync-*.sh` scripts and `run-cache.sh` under `/srv/mirroret/scripts/`
- master `sync-all.sh` and `cleanup-all.sh`
- `/etc/logrotate.d/mirroret`
- client configs under `/srv/mirroret/config/`
- the cron block
...
(the marker is the `mirroret-managed` line, `MIRRORET_MANAGED_MARKER`)
...
but backed up first - you can recover from
`/var/backups/mirroret/<timestamp>/`. The sentinel check applies to the
generated scripts under `/srv/mirroret/scripts/` and to
`/etc/logrotate.d/mirroret`.
...
- Still regenerates configs (nginx, systemd units, verdaccio.yaml, target
  specs, cache route table, client configs, logrotate)
- Still regenerates managed scripts (respecting the sentinel)
- Still refreshes the cron block
- Still runs the SELinux relabel
- Still (re)starts `mirroret-cache` in hybrid/cache mode, or disables it
  when switching back to `mirror`
...
- Doesn't run any sync. `sudo mirroretctl sync all` (or one ecosystem) if needed.
- Doesn't run retention. `sudo mirroretctl clean prune` / `--cleanup` for that.
